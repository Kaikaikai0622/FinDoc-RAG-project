...
- **模块化 pipeline 设计**：（Block → Flow → Contract），支持一键重新生成评估数据集，单组生成 50~60条问答成本稳定在 **¥1 以内**。
- **可持续评估闭环、成本可控**：pipeline 与 Hybrid Search 的 chunk 粒度完全对齐。
- 后续 Reranker、Router 等模块的收益均基于该数据集量化验证（见下方指标）

## 生成流程图
  ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐                                                                                                                     
  │ Block 1 │ → │ Block 2 │ → │ Block 3 │ → │ Block 4 │ → │ Block 6 │ → │ Block 5 │
  │ 主题提取 │   │ 问题生成 │   │ 问题演化 │   │ 答案生成 │   │ 数值校验 │   │ 质量过滤 │                                                                                                               
  └─────────┘   └─────────┘   └─────────┘   └─────────┘   └─────────┘   └─────────┘
       │              │              │              │              │              │
       ▼              ▼              ▼              ▼              ▼              ▼
   提取1-3个      生成可基于     改写为自然      基于文档       数值/单位      LLM评审
   核心主题词      文档回答的     语言风格，      生成答案，      一致性回查，    过滤幻觉和
                 初始问题      过滤口语称谓      标注场景难度     拦截幻觉       模糊QA
...
| **准确性**    | 准确率 (Accuracy)              | 0.7333 (↑16.7%) | 0.8692 (↑12%)   |
| **检索质量**   | 检索命中率 (Retrieval Hit Rate)  | 0.9333 (↑3.33%) | 0.9323 (↑ 4.2%) |
|            | 平均检索排名 (Avg Retrieval Rank) | 1.6786 (↑5.0%)  | 1.2690 (↓3.1%)  |
| **RAG 质量** | 忠实度 (Faithfulness)          | 0.8885 (↓5.52%) | 0.8963 (↑3.43%) |
|            | 回答相关性 (Answer Relevancy)    | 0.5085 (↑16.7%) | 0.6748 (↑13%)   |
|            | 上下文精确度 (Context Precision)  | 0.5214 (↑7.16%) | 0.7774 (↑11%)   |
|            | 上下文召回率 (Context Recall)     | 0.7489 (↑8.33%) | 0.9385 (↑9.7%)  |
